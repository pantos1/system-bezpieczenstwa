...
\noindent
Celem pracy jest stworzenie systemu bezpieczeństwa opartego o komputer jednopłytkowy z systemem Linux. Podstawowym zadaniem systemu jest realizacja monitoringu wizyjnego przez kamery internetowe dołączone do gniazda USB (ang. \textit{Universal Serial Bus}) komputera. System zrealizowano przy użyciu platformy Raspberry Pi 3B. Do wyprowadzeń GPIO (ang. \textit{General-Purpose Input/Output}) komputera dołączone są magnetyczne czujniki stykowe MC38 oraz multiplekser magistrali I2C (ang. \textit{Inter-Integrated Circuit}) TCA 9548A. Multiplekser umożliwia dołączenia wielu czujników temperatury Si7021, które posiadają taki sam, stały adres magistrali I2C. Program w języku Python wykonujący cyklicznie zdjęcia oraz odczyty czujników zapisuje zebrane dane w relacyjnej bazie danych stworzonej przy użyciu systemu zarządzania MySQL. 

Dostęp do danych zapewnia interfejs użytkownika oparty o stronę WWW umieszczoną na serwerze HTTP (ang. \textit{Hypertext Transfer Protocol}) Apache działającym na komputerze Raspberry Pi 3B. Komunikacja między klientem (przeglądarką internetową) a serwerem jest zrealizowana przez żądania protokołu HTTP w technice AJAX (ang. \textit{Asynchronous JavaScript and XML}). Interfejs użytkownika umożliwia dostęp do ostatnio wykonanych zdjęć, archiwum oraz panelu administracyjnego ustawień. Użytkownik może również przez interfejs zapisać do pliku listę stanów systemu.

\vspace{11pt}
\noindent
\textbf{Słowa kluczowe:} monitoring wizyjny, komputer jednopłytkowy, Linux, kamery USB, czujniki stykowe, czujniki warunków środowiskowych, I2C, serwer HTTP, strona WWW
...
\noindent
The aim of this thesis is to design a security system using single-board computer with Linux operating system. The system's primary objective is conducting video surveillance through web cameras connected to the \textit{Universal Serial Bus} (USB) ports of the computer. The system was designed using Raspberry Pi 3B platform. Magnetic reed switches MC38 and an \textit{Inter-Integrated Circuit} (I2C) bus multiplexer TCA 9548A are connected to the \textit{General-Purpose Input/Output} (GPIO) pins of the computer. Thanks to the multiplexer it is possible to connect multiple temperature and relative humidity sensors Si7021, which have the same, fixed I2C address. Python program periodically takes photographs and performs measurements, saving the results in relational database created with management system MySQL.

The access to the data is possible through the user’s interface on the website hosted on the Apache web server running on the Raspberry Pi 3B. The communication between the client (web browser) and the server is accomplished with \textit{Hypertext Transfer Protocol} (HTTP) protocol in \textit{Asynchronous JavaScript and XML} (AJAX) technique. The user’s interface displays current and past photographs and allows access to the settings panel. The user may also download a file with the system’s logs through the website.

\vspace{11pt}
\noindent
\textbf{Keywords:} video surveillance, single-board computer, Linux, USB cameras, reed switches, environmental sensors, I2C, web server, website 
\newpage
...
Ważnym aspektem systemu zarządzającego inteligentnym domem jest również możliwość sterowania systemem na odległość np. przez sieć Internet. Stawia to wymóg posiadania wydajnego i zabezpieczonego interfejsu użytkownika, przez który użytkownik będzie miał dostęp do stanu budynku i możliwość zdalnej kontroli  domowej automatyki. Pozwala to ograniczyć niepokój o pozostawione mienie np. w czasie dłuższego wyjazdu. Związane są z tym bardziej potrzeby leżące u podstaw koncepcji inteligentnego domu -- poprawa komfortu życia przez automatyczne zarządzanie budynkiem oraz zwiększenie poczucia bezpieczeństwa dzięki kontroli dostępu do niego. 

\subsection{Motywacje i cel pracy}
Opisane wcześniej systemy zarządzania inteligentnymi budynkami oferują bardzo duże możliwości, ale jedną z ich wad jest wysoki koszt wykonania. Mogą być więc ograniczeniem dla osób chcących poprawić bezpieczeństwo swojego budynku, ale niedysponujących odpowiednimi środkami. Przykładowo koszt jednej kamery IP to ok. 400 zł \cite{komputer_świat}. Ponadto funkcjonalność komercyjnych systemów jest z reguły ograniczona do pewnego zakresu funkcji przewidzianych przez producenta i użytkownik nie ma możliwości ich rozbudowy. Gdyby jednak zastąpić je kamerami internetowymi korzystającymi z magistrali USB (ang. \textit{Universal Serial Bus}), możliwe byłoby zmniejszenie kosztu monitoringu wizyjnego. Konieczna byłaby jednak wówczas platforma z odpowiednim oprogramowaniem, która sprawowałaby nadzór przy użyciu tych kamer. 

Niniejsza praca ma więc na celu stworzenie domowego systemu bezpieczeństwa, który będzie realizował funkcje monitoringu wizyjnego oraz kontroli dostępu wraz z pomiarem warunków środowiskowych wpływających na komfort przebywania w pomieszczaniach. System ten powinien umożliwiać sieciowy dostęp do zebranych danych. Dzięki temu po podłączeniu go do domowej sieci Internet możliwy będzie zdalny nadzór nad budynkiem. 

To zadanie wymaga integracji wielu obszarów elektroniki i informatyki: od stworzenia prototypu sprzętu, przez właściwą obsługę programową dołączonych urządzeń, aż po technologie sieciowe związane z zapewnieniem dostępu do danych. Jednocześnie stwarza to okazję do poznania wielu technologii dzięki różnorodnemu zakresowi zadań do wykonania. Ponadto wykonany system będzie mógł być użyty w praktyce w warunkach domowych i w przyszłości rozbudowany o kolejne funkcje, w miarę potrzeb.
...
\begin{table}[h]
\centering
\caption{Definicja bajtu sterującego}
\small
...
Wszystkie wpisy okazały się poprawne. Następnie sprawdzono, czy dane są poprawnie przekazywane z serwera HTTP do przeglądarki internetowej. Po otwarciu strony WWW i uwierzytelnieniu w systemie można otworzyć zakładkę deweloperską  
